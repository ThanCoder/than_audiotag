...
class TagLibPicture {
  ...
  CoverArt? read() {
    ...
    try {
      final properties = taglib_complex_property_get(file, keyPtr.cast());
  ...
  void write(
    ...
    try {
      ...
      final result = taglib_complex_property_set(file, keyPtr.cast(), attrs);
      if (result == 0) {
  ...
  void remove() {
    final keyPtr = 'PICTURE'.toNativeUtf8();
    try {
      final result = taglib_complex_property_set(
        file,
        keyPtr.cast(),
        ffi.nullptr,
      );
